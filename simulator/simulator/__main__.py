...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser("siem-simulator", description=__doc__,
                                formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("--rate", type=int, default=CONFIG.events_per_second,
                   help="events per second (default from env)")
    p.add_argument("--scenario", choices=SCENARIO_CHOICES, default=CONFIG.scenario)
    p.add_argument("--attack-ratio", type=float, default=CONFIG.attack_ratio,
                   help="fraction of malicious traffic in 'mixed' scenario")
    p.add_argument("--sink", choices=["kafka", "console"], default=CONFIG.sink)
    p.add_argument("--once", metavar="SCENARIO", choices=sorted(SCENARIOS),
                   help="emit one instance of SCENARIO and exit")
    p.add_argument("--no-redis-control", action="store_true",
...
async def _run(args: argparse.Namespace) -> None:
    ...
    if args.once:
        ...
        print(f"emitted {n} events for scenario '{args.once}'")  # noqa: T201
        return
